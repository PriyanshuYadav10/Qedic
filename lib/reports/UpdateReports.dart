...
class UpdateReports extends StatefulWidget {
  Data_report data_report;
  UpdateReports({required this.data_report});
  @override
  _UpdateReportsState createState() => _UpdateReportsState();
}

class _UpdateReportsState extends State<UpdateReports> {
  ...
  void autofildata() {
    _selecteStartdate = widget.data_report.startDate!;
    _selectedEnddate = widget.data_report.endDate!;
    title_Controller.text = widget.data_report.title!;
    startdate_Controller.text = widget.data_report.startDate!;
    endtdate_Controller.text = widget.data_report.endDate!;
    remarks_Controller.text = widget.data_report.remark!;
    for(var item in widget.data_report.submitExpenses!){
      List<Expenses>? expenses = item.expenses;
      for(var item2 in expenses!){
        ...
        data_expenses.selectDate = item2.selectDate!;
        data_expenses.travelPurpose = item2.travelPurpose;
        data_expenses.toLocation = item2.toLocation;
        data_expenses.mileageKm = item2.mileageKm;
        data_expenses.fromLocation = item2.fromLocation;
        data_expenses.routeType = item2.routeType;
        data_expenses.mileageAllowance = item2.mileageAllowance;
        data_expenses.hotelExpReceipt = item2.hotelExpReceipt;
        data_expenses.vechileFare = item2.vechileFare;
        data_expenses.vechileReceipt = item2.vechileReceipt;
        data_expenses.otherExpReceipt = item2.otherExpReceipt;
        data_expenses.accountStatus = item2.accountStatus;
        data_expenses.isEditable = item2.isEditable;
        data_expenses.adminComment = item2.adminComment;
        data_expenses.travelWithMd = item2.travelWithMd;
        data_expenses.routeType = item2.routeType;
        data_expenses.remark = item2.remark;
        data_expenses.status = item2.status;
        data_expenses.otherExp = item2.otherExp;
        data_expenses.hotelExp = item2.hotelExp;
        data_expenses.remark = item2.remark;
        data_expenses.status = item2.status;
        data_expenses.adminComment = item2.adminComment;
  ...
  _updateAddreport(BuildContext _context) async {
    LoginModel loginModel = await Commons.getuser_info();
    _context.loaderOverlay.show();
    try {
      //create multipart request for POST or PATCH method
      var request = http.MultipartRequest("POST", Uri.parse(Commons.editreport));
      ...
      request.fields["report_id"] = widget.data_report.id.toString();
