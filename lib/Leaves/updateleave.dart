...
class UpdateLeave extends StatefulWidget {
  ...
  final Data_leave data_leave;

  const UpdateLeave({Key? key, required this.data_leave})
      : super(key: key);
  @override
  State<StatefulWidget> createState() => _UpdateLeave();
}

class _UpdateLeave extends State<UpdateLeave> {
  var OneDay = true;
  ...
  List<bool> leavecheck = [false, false, false, false, false, false];

  List<String> daytypelist = ["Half Day", "Full Day", "Multiple Days"];
  List<bool> daycheck = [false, false, false];
  int leavelastindex = -1;
  int dayslastindex = -1;
  var leave_days;
  ...
  // Auto filling the
  void autofill() {
    if (widget.data_leave.leaveType == "Casual") {
      setState(() {
        leavecheck[0] = true;
        leavelastindex=0;
      });
    } else if (widget.data_leave.leaveType == "Sick") {
      setState(() {
        leavecheck[1] = true;
        leavelastindex=1;

      });
    } else if (widget.data_leave.leaveType == "Earned") {
      setState(() {
        leavecheck[2] = true;
        leavelastindex=2;
      });
    } else if (widget.data_leave.leaveType == "Maternity") {
      setState(() {
        leavecheck[3] = true;
        leavelastindex=3;
      });
    } else if (widget.data_leave.leaveType == "Paternity") {
      setState(() {
    ...
    } else if (widget.data_leave.leaveType == "Other") {
      setState(() {
        leavecheck[0] = false;
        leavecheck[1] = false;
        leavecheck[2] = false;
    ...
    if (widget.data_leave.leaveData == "Half Day") {
      setState(() {
        daycheck[0] = true;
        dayslastindex=0;
      });
    } else if (widget.data_leave.leaveData == "Full Day") {
      setState(() {
        daycheck[1] = true;
        dayslastindex=1;
      });
    } else if (widget.data_leave.leaveData == "Multiple Days") {
      setState(() {
        dayslastindex=2;
        daycheck[2] = true;
        OneDay = false;
    ...
    description_Controller.text = widget.data_leave.description!;
    newstartDate = widget.data_leave.startDate!;
    newendDate = widget.data_leave.endDate!;
    selected_start_date = widget.data_leave.startDate!;
    newendDateymd = widget.data_leave.endDate!;
    day_type = widget.data_leave.leaveData!;
  }
  ...
  RequestLeaveAPI() async {
    ...
    try {
      ...
      var request = http.MultipartRequest("POST", Uri.parse(Commons.updateLeave));
      //add text fields
      request.fields["user_id"] = "${loginModel.data!.id ??""}";
      request.fields["leave_id"] = widget.data_leave.id??"";
