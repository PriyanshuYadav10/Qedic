...
class UpdateExpenses extends StatefulWidget {
  ...
  final Data_Expenses data_expenses;

  const UpdateExpenses({Key? key, required this.data_expenses})
      : super(key: key);
...
class _UpdateExpensesState extends State<UpdateExpenses> {
  ...
  void Autofilldata() {
    setState(() {
      ...
      km_controller.text = widget.data_expenses.mileageKm.toString();
      mileage_controller.text =
          widget.data_expenses.mileageAllowance.toString();

      HotelExpense_controller.text = widget.data_expenses.hotelExp.toString();
      // _HotelExpenseImage_url = widget.data_expenses.hotelExpReceipt.toString();
      if (widget.data_expenses.hotelExpReceipt!.isNotEmpty) {
        hotel_image_list
            .addAll(widget.data_expenses.hotelExpReceipt.toString().split(","));
      }

      bus_train_controller.text = widget.data_expenses.vechileFare.toString();
      // _BusTrain_url = widget.data_expenses.vechileReceipt.toString();
      if (widget.data_expenses.vechileReceipt!.isNotEmpty) {
        travel_image_list
            .addAll(widget.data_expenses.vechileReceipt.toString().split(","));
      }
      // travel_image_list
      //     .addAll(widget.data_expenses.vechileReceipt.toString().split(","));

      other_controller.text = widget.data_expenses.otherExp.toString();
      remark_supportController.text = widget.data_expenses.remark.toString();
      // _Other_url = widget.data_expenses.otherExpReceipt.toString();
      if (widget.data_expenses.otherExpReceipt!.isNotEmpty) {
        other_image_list
            .addAll(widget.data_expenses.otherExpReceipt.toString().split(","));
      }
      // other_image_list
      //     .addAll(widget.data_expenses.otherExpReceipt.toString().split(","));
      Traveltype = widget.data_expenses.travelWithMd.toString();
      routetype = widget.data_expenses.routeType.toString();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Container(
        child: SafeArea(
            child: Scaffold(
                appBar: Commons.Appbar_logo(true, context, "Update Expenses"),
                body: LoaderOverlay(
                        ...
                        onTap: () async {
                          ...
                          if (result != null) {
                            ...
                            setState(() {
                              ...
                              print("sarjeet $visit_id_selected");
                            });
                          }
                        },
                      ),
  ...
  bool _Validation(BuildContext _context) {
    ...
    // else if (formlocation_controller.text.isEmpty) {
  ...
  _UpdateExpenses(BuildContext _context) async {
    ...
    try {
      //create multipart request for POST or PATCH method
      var request =
          http.MultipartRequest("POST", Uri.parse(Commons.updateExpenses));
      //add text fields
      ...
      // request.fields["routetype"] = other_exp_receipt;
